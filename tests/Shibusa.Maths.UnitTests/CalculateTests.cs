...
namespace Shibusa.Maths.UnitTests
{
    ...
    {
        private readonly List<ulong> predefinedPrimes = new() {
        ...
        public void AgeDateTime(string birthDateString, string fromDateString, int expected)
        {
        ...
        [InlineData("1971-01-01", "1970-01-01", -1)]
        [InlineData("2012-02-29", "2008-02-29", -4)]
        public void AgeDateOnly(string birthDateString, string fromDateString, int expected)
        {
            DateOnly birthDate = DateOnly.ParseExact(birthDateString, "yyyy-MM-dd", CultureInfo.InvariantCulture);
            DateOnly fromDate = DateOnly.ParseExact(fromDateString, "yyyy-MM-dd", CultureInfo.InvariantCulture);
            Assert.Equal(expected, Calculate.AgeInYears(birthDate, fromDate));
        }
        ...
        public void GetPrimes_ULong()
        {
            ...
            ulong highestPredefinedPrime = predefinedPrimes.Last();
        ...
        {
            IDictionary<ulong, int>? actuals = Calculate.GetPrimeFactorsDictionary(1);
            Assert.Empty(actuals);
        ...
        {
            IEnumerable<ulong>? actuals = Calculate.GetPrimeFactors(1);
            Assert.Empty(actuals);
        ...
        {
            IEnumerable<ulong>? fibs = Calculate.GetFibonacciNumbersByQuantity(6);
            List<ulong>? expected = new() { 0L, 1L, 1L, 2L, 3L, 5L };
        ...
        public void GetFibonacciNumbersUpToValue()
        {
            ...
            IEnumerable<ulong>? seq = Calculate.GetFibonacciNumbersUpToMaxValue(9);
